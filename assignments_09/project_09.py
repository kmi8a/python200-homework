...
print(f"Request succeeded: {response.ok}")
print(f"Top-level keys in JSON: {list(data.keys())}")
daily_times = data.get("daily", {}).get("time", [])
print(
    f"Date range returned: {daily_times[0] if daily_times else 'N/A'} to"
    f" {daily_times[-1] if daily_times else 'N/A'}"
)
print(f"Number of daily records: {len(daily_times)}\n")
...
print("Earliest record:", earliest.data[0] if earliest.data else "None")
print("Latest record:", latest.data[0] if latest.data else "None")
